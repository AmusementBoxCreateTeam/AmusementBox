...
public partial class MMD4MecanimImporter : ScriptableObject
{
	...
	public class IndexBuilder
	{
		...
		public const int MeshCountBitShift = MMD4MecanimAuxData.IndexData.MeshCountBitShift;
	...
	public class VertexBuilder
	{
		...
		public const int MeshCountBitShift = MMD4MecanimAuxData.VertexData.MeshCountBitShift;
				...
				for( int meshIndex = 0; meshIndex < _cache_skinnedMeshRenderers.Length; ++meshIndex ) {
					...
					for( int i = 0; i < boneCount; ++i ) {
						...
						if( bindpose.GetColumn( 0 ) == new Vector4(1,0,0,0) &&
							...
							boneAttribute |= (byte)MMD4MecanimAuxData.VertexData.BoneFlags.OptimizeBindPoses;
						}
					...
					for( int i = 0; i < colors.Length; ++i ) {
						...
						if( vertexIndex < vertexCount ) {
							...
							vertexAttribute |= (isSDEF ? (byte)MMD4MecanimAuxData.VertexData.VertexFlags.SDEF : (byte)0);
							vertexAttribute |= (isQDEF ? (byte)MMD4MecanimAuxData.VertexData.VertexFlags.QDEF : (byte)0);
							if( isSDEF ) {
								...
								if( originalBoneIndex0 == vertexData.boneWeight.boneIndex0 &&
									...
									vertexAttribute |= (byte)MMD4MecanimAuxData.VertexData.VertexFlags.SDEFSwapIndex;
								} else {
								...
								byte flag = (byte)MMD4MecanimAuxData.VertexData.BoneFlags.SDEF;
							...
							if( isQDEF ) {
								byte flag = (byte)MMD4MecanimAuxData.VertexData.BoneFlags.QDEF;
								if( boneWeights[i].boneIndex0 >= 0 ) {
									meshBoneAttributes[baseBoneIndex + boneWeights[i].boneIndex0] |= flag;
								}
								if( boneWeights[i].boneIndex1 >= 0 ) {
							...
							if( isSDEF ) {
								...
								if( (vertexAttribute & (byte)MMD4MecanimAuxData.VertexData.VertexFlags.SDEFSwapIndex) != 0 ) {
					...
					int meshAttribute = 0;
					meshAttribute |= (isAnySDEF ? (int)MMD4MecanimAuxData.VertexData.MeshFlags.SDEF : 0);
					meshAttribute |= (isAnyQDEF ? (int)MMD4MecanimAuxData.VertexData.MeshFlags.QDEF : 0);
	...
	bool _CheckFBXMaterialIndex( SkinnedMeshRenderer[] skinnedMeshRenderers )
	{
		...
		if( skinnedMeshRenderers != null && skinnedMeshRenderers.Length > 0 ) {
			...
			if( this.indexData != null ) {
				if( !MMD4MecanimAuxData.ValidateIndexData( this.indexData, skinnedMeshRenderers ) ) {
					this.indexData = null;
			...
			if( initializeIndex ) {
				...
				if( this.indexAsset != null ) {
					this.indexData = MMD4MecanimAuxData.BuildIndexData( this.indexAsset );
				}
			}
	...
	bool _CheckFBXMaterialVertex( SkinnedMeshRenderer[] skinnedMeshRenderers )
	{
		...
		if( skinnedMeshRenderers != null && skinnedMeshRenderers.Length > 0 ) {
			...
			if( this.vertexData != null ) {
				if( !MMD4MecanimAuxData.ValidateVertexData( this.vertexData, skinnedMeshRenderers ) ) {
			...
			if( initializeVertex ) {
				...
				if( this.vertexAsset != null ) {
					this.vertexData = MMD4MecanimAuxData.BuildVertexData( this.vertexAsset );
				}
			}
