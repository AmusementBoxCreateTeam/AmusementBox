...
public partial class MMD4MecanimImporter : ScriptableObject
{
	...
	public void OnInspectorGUI()
	{
		...
		if( this.pmx2fbxConfig != null ) {
			if( _licenseAgree == LicenseAgree.NotProcess ) {
				_licenseAgree = CheckLicenseAgree( ref _readmeFiles, ref _readmeTexts );
			...
			if( !_isAgreedWeak ) {
...

				{
					...
					if( _licenseAgree != LicenseAgree.Denied ) {
						EditorGUILayout.SelectableLabel( LicenceWarningNotice );
						EditorGUILayout.Separator();
					}
					_isCheckedLicense = GUILayout.Toggle(_isCheckedLicense, LicenceAgreedWarning);
					EditorGUILayout.Separator();
					if( _licenseAgree == LicenseAgree.Denied ) {
						_isCheckedWarning = GUILayout.Toggle(_isCheckedWarning, LicenceDeniedWarning);
						EditorGUILayout.TextArea( LicenceDeniedNotice );
						EditorGUILayout.Separator();
						GUI.enabled = _isCheckedLicense && _isCheckedWarning;
					} else {
						GUI.enabled = _isCheckedLicense;
					}
					EditorGUILayout.BeginHorizontal();
					GUILayout.FlexibleSpace();
		...
		if( this.fbxAsset != null ) {
			if( this.indexAsset == null || this.indexData == null ) {
				...
				if( File.Exists( indexAssetPath ) ) {
					...
					if( indexAsset != null ) {
						this.indexData = MMD4MecanimAuxData.BuildIndexData( indexAsset );
			...
			if( this.vertexAsset == null || this.vertexData == null ) {
				...
				if( File.Exists( vertexAssetPath ) ) {
					...
					if( vertexAsset != null ) {
						this.vertexData = MMD4MecanimAuxData.BuildVertexData( vertexAsset );
	...
	public LicenseAgree CheckLicenseAgree( ref string[] readmeFiles, ref string[] readmeTexts )
	{
		readmeFiles = FindReadmeFiles();
		readmeTexts = null;
		return CheckLicenseAgree( readmeFiles, ref readmeTexts );
	}
	...
	public static LicenseAgree CheckLicenseAgree( string[] readmeFiles, ref string[] readmeTexts )
	{
		readmeTexts = null;
		if( readmeFiles == null ) {
