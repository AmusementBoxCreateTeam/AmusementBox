...
public class MMD4MecanimBulletPMXRigidBody
{
	...
	public bool isFreezed { get { return _isFreezed; } }
	...
	public int originalParentBoneID { get { return (_bone != null && _bone.originalParentBone != null) ? _bone.originalParentBone.boneID : -1; } }
	...
	public bool Import( MMD4MecanimCommon.BinaryReader binaryReader )
	{
		...
		_isFreezed			= (_additionalFlags & 0x01) != 0;
	...
	public void FeedbackBoneToBodyTransform( bool forceOverwrite )
	{
		...
		} else if( forceOverwrite || _isFreezed ) {
			if( _bulletRigidBody != null ) {
	...
	public void ProcessPreBoneAlignment()
	{
		...
		if( _rigidBodyType == PMXRigidBodyType.Kinematics || _isFreezed ) {
			return;
		}
	...
	{
		if( _rigidBodyType == PMXRigidBodyType.Kinematics || _bone == null || _noBone || _isFreezed || _model == null || _modelProperty == null ){
			return;
		}
	...
	public void PrepareCollider()
	{
		...
		_collider.isKinematic	= _rigidBodyType == PMXRigidBodyType.Kinematics || _isFreezed;
	...
	{
		if( _bone != null && !_noBone ) {
			_bone.SetWorldTransformFromBody( ref boneTransform );
		}
	}
		...
		if( _bulletRigidBody != null ) {
			...
			if( _model != null && !_model.optimizeBulletXNA ) {
				if( _modelProperty != null && _modelProperty.rigidBodyIsUseCcd ) {
					...
					float ccdThreshold = _modelProperty.rigidBodyCcdMotionThreshold * _model.worldToBulletScale;
	...
	void _ProcessVelocityLimit()
	{
		...
		if( _rigidBodyProperty.linearVelocityLimit >= 0.0f ) {
			float limitValue = _rigidBodyProperty.linearVelocityLimit * _model.worldToBulletScale;
		...
		float lossRate )
	{
		...
		limitLength *= model.worldToBulletScale;
	
		if( _bone.originalParentBone != null ) {
			Vector3 position = boneTransform._origin;
			Vector3 parentPosition = _bone.originalParentBone.worldTransform._origin;
			Vector3 translate = position - parentPosition;
			Vector3 offset = _bone.originalParentBone.worldTransform._basis.Transpose() * translate;
			...
			if( tempLength > limitLength ) { // pending: optimized
				...
				Vector3 translate2 = _bone.originalParentBone.worldTransform._basis * offset2;
		...
		float deltaTime )
	{
		...
		if( _rigidBodyProperty.isUseForceAngularVelocityLimit != 0 && deltaTime > 0.0f && _bone.originalParentBone != null ) {
