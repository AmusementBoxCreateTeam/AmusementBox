...
public class MMD4MecanimBulletPhysics : MonoBehaviour
{
	...
	{
		public float	worldScale										= 0.0f;
		public float	importScale										= 0.0f;
		...
		public void Postfix()
		{
			if( keepIKTargetBoneFlag == -1 )				keepIKTargetBoneFlag			= 0;
			if( enableIKInnerLockFlag == -1 )				enableIKInnerLockFlag			= 1;
			if( enableIKInnerLockKneeFlag == -1 )			enableIKInnerLockKneeFlag		= 1;
			if( innerLockKneeClamp < 0.0f )					innerLockKneeClamp				= 1.0f / 16.0f;
			if( innerLockKneeRatioL < 0.0f )				innerLockKneeRatioL				= 0.4f;
			if( innerLockKneeRatioU < 0.0f )				innerLockKneeRatioU				= 0.1f;
			if( innerLockKneeScale < 0.0f )					innerLockKneeScale				= 8.0f;
			
			if( enableIKMuscleFlag == -1 )					enableIKMuscleFlag				= 1;
			if( enableIKMuscleHipFlag == -1 )				enableIKMuscleHipFlag			= 1;
			if( enableIKMuscleFootFlag == -1 )				enableIKMuscleFootFlag			= 1;
			
			if( muscleHipUpperXAngle < 0.0f )				muscleHipUpperXAngle			= 176.0f;
			if( muscleHipLowerXAngle < 0.0f )				muscleHipLowerXAngle			= 86.0f;
			if( muscleHipInnerYAngle < 0.0f )				muscleHipInnerYAngle			= 45.0f;
			if( muscleHipOuterYAngle < 0.0f )				muscleHipOuterYAngle			= 90.0f;
			if( muscleHipInnerZAngle < 0.0f )				muscleHipInnerZAngle			= 30.0f;
			if( muscleHipOuterZAngle < 0.0f )				muscleHipOuterZAngle			= 90.0f;
			
			if( muscleFootUpperXAngle < 0.0f )				muscleFootUpperXAngle			= 70.0f;
			if( muscleFootLowerXAngle < 0.0f )				muscleFootLowerXAngle			= 90.0f;
			if( muscleFootInnerYAngle < 0.0f )				muscleFootInnerYAngle			= 25.0f;
			if( muscleFootOuterYAngle < 0.0f )				muscleFootOuterYAngle			= 25.0f;
			if( muscleFootInnerZAngle < 0.0f )				muscleFootInnerZAngle			= 12.5f;
			if( muscleFootOuterZAngle < 0.0f )				muscleFootOuterZAngle			= 0.0f;

			muscleHipUpperXAngle *= Mathf.Deg2Rad;
			muscleHipLowerXAngle *= Mathf.Deg2Rad;
			muscleHipInnerYAngle *= Mathf.Deg2Rad;
			muscleHipOuterYAngle *= Mathf.Deg2Rad;
			muscleHipInnerZAngle *= Mathf.Deg2Rad;
			muscleHipOuterZAngle *= Mathf.Deg2Rad;

			muscleFootUpperXAngle *= Mathf.Deg2Rad;
			muscleFootLowerXAngle *= Mathf.Deg2Rad;
			muscleFootInnerYAngle *= Mathf.Deg2Rad;
			muscleFootOuterYAngle *= Mathf.Deg2Rad;
			muscleFootInnerZAngle *= Mathf.Deg2Rad;
			muscleFootOuterZAngle *= Mathf.Deg2Rad;
		}
	};
	...
	public class MMDRigidBodyProperty
	{
		...
		public void Copy( MMDRigidBodyProperty rhs )
		{
			...
			if( rhs.isFreezed != -1 )							this.isFreezed = rhs.isFreezed;
	...
	public class MMDModel
	{
				...
				for( int i = 0; i != meshFlags.Length; ++i ) {
					if( (meshFlags[i] & (int)(MeshFlags.VertexMorph | MeshFlags.XDEF)) != 0 ) {
						...
						if( cloneMesh != null ) {
							...
							this.bulletPMXModel.UploadMesh( i, vertices, normals, boneWeights, bindposes );
						}
					}
				}
			}
		...
		public void Update()
		{
						...
						if( bone != null ) {
							...
							if( !bone._userPositionIsZero != positionIsValid ) {
								...
								if( bone._userPositionIsZero ) {
									_updateUserPositionIsValidList[i >> 5] &= ~(1u << (1 & 0x1f));
									_updateUserPositionList[i] = Vector3.zero;
								} else {
									_updateUserPositionIsValidList[i >> 5] |= (1u << (1 & 0x1f));
							...
							if( !bone._userRotationIsIdentity != rotationIsValid ) {
								...
								if( bone._userRotationIsIdentity ) {
									_updateUserRotationIsValidList[i >> 5] &= ~(1u << (1 & 0x1f));
									_updateUserRotationList[i] = Quaternion.identity;
								} else {
									_updateUserRotationIsValidList[i >> 5] |= (1u << (1 & 0x1f));
			...
			if( this.bulletPMXModel != null ) {
				unchecked {
					this.bulletPMXModel.Update( (uint)updateFlags,
		...
		public void LateUpdate( float deltaTime )
		{
			...
			#if !NOUSE_BULLETXNA_UNITY
			MMD4MecanimBulletPMXModel.UpdateData updateDataBulletXNA = null;
			if( this.bulletPMXModel != null ) {
				updateDataBulletXNA = this.bulletPMXModel.LateUpdate();
			...
			unchecked {
				...
				if( (lateUpdateFlags & (int)LateUpdateFlags.Mesh) != 0 && lateUpdateMeshFlagsList != null ) {
					#if !NOUSE_BULLETXNA_UNITY
					if( this.bulletPMXModel != null && updateDataBulletXNA != null ) {
						MMD4MecanimBulletPMXModel.LateUpdateMeshData[] lateUpdateMeshData = updateDataBulletXNA.lateUpdateMeshDataList;
